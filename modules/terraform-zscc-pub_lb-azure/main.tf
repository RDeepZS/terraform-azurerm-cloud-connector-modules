...
resource "azurerm_public_ip" "frontend_ip" {
  name                = "${var.name_prefix}-pub-cc-lb-${var.resource_tag}-ip"
...
resource "azurerm_lb" "cc_lb" {
  name                = "${var.name_prefix}-pub-cc-lb-${var.resource_tag}"
  location            = var.location
  resource_group_name = var.resource_group
  sku                 = "Standard"
...
resource "azurerm_lb_backend_address_pool" "cc_lb_backend_pool" {
  name            = "${var.name_prefix}-cc-pub_lb-backend-${var.resource_tag}"
...
resource "azurerm_lb_rule" "cc_lb_rule" {
  ...
  frontend_ip_configuration_name = "${var.name_prefix}-pub-cc-lb-${var.resource_tag}-ip"
...
resource "azurerm_lb_rule" "cc_lb_rule_443" {
  name                           = "${var.name_prefix}-cc-lb-rule-443-${var.resource_tag}"
  loadbalancer_id                = azurerm_lb.cc_lb.id
  protocol                       = "Tcp"
  frontend_port                  = 443
  backend_port                   = 443
